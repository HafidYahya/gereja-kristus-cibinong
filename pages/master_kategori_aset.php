...
// routing
$currentPage = $_GET['page'] ?? 'master_kategori_aset';
// pagination
...
$sqlTotal = "SELECT COUNT(*) as total FROM master_kategori_aset $whereSql";
...
$master_kategori_aset = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
...
<div class="p-4">
    <div class="container border border-warning border-top-0 border-bottom-0 bg-white p-3 rounded shadow mb-4">
        <div class="page-header row g-2 align-items-center">
            <div class="col-12 col-sm">
                <h1 class="page-title mb-0">Master Kategori Aset</h1>
            </div>
            <div class="col-12 col-sm-auto">
                <button type="button"
                    ...
                    data-bs-toggle="modal" data-bs-target="#modal-tambah-master-kategori"><i
            ...
            <div class="col-sm-12 col-lg-4">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control border-start-0 border border-warning"
                        placeholder="Cari nama kategori aset" aria-label="Search" aria-describedby="search"
    ...
    <div class=" table-responsive border border-warning rounded bg-white">
        <div class="table-container">
            <table class="table table-hover text-nowrap shadow-md" style="min-width:700px;">
                ...
                <tbody>
                    <?php $no = $offset + 1; ?>
                    <?php if (empty($master_kategori_aset)) : ?>
                        <tr>
                    ...
                    <?php else : ?>
                        <?php foreach ($master_kategori_aset as $data) : ?>
                            <tr>
                                <td><?= $no++ ?></td>
    ...
    <nav class="mt-3">
        <ul class="pagination justify-content-center">
            ...
            <li class="page-item <?= ($p <= 1) ? 'disabled' : '' ?>">
                <a class="page-link"
                    href="index.php?page=master_kategori_aset&p=<?= $p - 1 ?>&filter=<?= urlencode($filter) ?>&search=<?= urlencode($search) ?>">
                    Previous
                </a>
            </li>
            ...
            <?php for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= ($i == $p) ? 'active' : '' ?>">
                    <a class="page-link"
                        href="index.php?page=master_kategori_aset&p=<?= $i ?>&filter=<?= urlencode($filter) ?>&search=<?= urlencode($search) ?>">
                        <?= $i ?>
                    </a>
                </li>
            <?php endfor; ?>
    ...
    <!-- MODAL TAMBAH -->
    <div class="modal fade" id="modal-tambah-master-kategori" tabindex="-1" aria-hidden="true"
        aria-labelledby="modal-tambah-master-kategori-label">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title" id="modal-tambah-master-kategori-label">Tambah Master Kategori Aset</h5>
