...
// routing
$currentPage = $_GET['page'] ?? 'master_kelompok_aset';
// pagination
...
$sql = "SELECT * FROM master_kelompok_aset $whereSql ORDER BY kel_is_active DESC, id DESC LIMIT ? OFFSET ?";
...
$sqlTotal = "SELECT COUNT(*) as total FROM master_kelompok_aset $whereSql";
...
$master_kelompok_aset = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
...
<div class="p-4">
    <div class="container border border-warning border-top-0 border-bottom-0 bg-white p-3 rounded shadow mb-4">
        <div class="page-header row g-2 align-items-center">
            <div class="col-12 col-sm">
                <h1 class="page-title mb-0">Master Kelompok Aset</h1>
            </div>
            <div class="col-12 col-sm-auto">
                <button type="button"
                    ...
                    data-bs-toggle="modal" data-bs-target="#modal-tambah-master-kelompok"><i
    ...
    <form method="GET">
        <div class="row mb-3">
            <div class="col-sm-12 col-lg-4">
                ...
                <input type="hidden" name="page" value="master_kelompok_aset">
            ...
            <div class="col-sm-12 col-lg-4">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control border-start-0 border border-warning"
                        placeholder="Cari nama kelompok aset" aria-label="Search" aria-describedby="search"
    ...
    <div class=" table-responsive border border-warning rounded bg-white">
        <div class="table-container">
            <table class="table table-hover text-nowrap shadow-md" style="min-width:700px;">
                ...
                <tbody>
                    <?php $no = $offset + 1; ?>
                    <?php if (empty($master_kelompok_aset)) : ?>
                        <tr>
                    ...
                    <?php else : ?>
                        <?php foreach ($master_kelompok_aset as $data) : ?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td><?= htmlspecialchars($data['kel_nama']) ?></td>
                                <td>
                                    <span style="min-width: 100px"
                                        class="badge badge-sm <?= (int) $data['kel_is_active'] === 1 ? 'bg-success' : 'bg-danger' ?>">
                                        <?= (int) $data['kel_is_active'] === 1 ? 'Aktif' : 'Tidak Aktif' ?>
                                    </span>
                                </td>
                                <td>
                                    <form method="post" class="form-confirm-ubah-status"
                                        ...
                                        <button style="min-width: 100px" type="submit"
                                            class="btn btn-sm  <?= (int) $data['kel_is_active'] === 1 ? 'btn-danger' : 'btn-success' ?>"><?= (int) $data["kel_is_active"] === 1 ? 'Nonaktifkan' : 'Aktifkan'  ?></button>
                                    </form>
    ...
    <nav class="mt-3">
        <ul class="pagination justify-content-center">
            ...
            <li class="page-item <?= ($p <= 1) ? 'disabled' : '' ?>">
                <a class="page-link"
                    href="index.php?page=master_kelompok_aset&p=<?= $p - 1 ?>&filter=<?= urlencode($filter) ?>&search=<?= urlencode($search) ?>">
                    Previous
                </a>
            </li>
            ...
            <?php for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= ($i == $p) ? 'active' : '' ?>">
                    <a class="page-link"
                        href="index.php?page=master_kelompok_aset&p=<?= $i ?>&filter=<?= urlencode($filter) ?>&search=<?= urlencode($search) ?>">
                        <?= $i ?>
                    </a>
                </li>
            <?php endfor; ?>
    ...
    <!-- MODAL TAMBAH -->
    <div class="modal fade" id="modal-tambah-master-kelompok" tabindex="-1" aria-hidden="true"
        aria-labelledby="modal-tambah-master-kelompok-label">
            ...
            <div class="modal-content">
                ...
                <form action="../actions/master_kelompok_aset/tambah.php" method="post" class="form-confirm-tambah">
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Nama Master Kelompok Aset*</label>
                            <input type="text" class="form-control" name="nama"
                                placeholder="Masukan nama master kelompok aset" required>
                        </div>
